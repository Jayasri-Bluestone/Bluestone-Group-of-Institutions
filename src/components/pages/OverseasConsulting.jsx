...
import { ArrowLeft, Plane, Globe, Users, Award, CheckCircle, TrendingUp, BookOpen, MapPin, Star, ShieldCheck } from 'lucide-react';
import { motion } from 'framer-motion';
import { Button } from '../ui/button';

export function OverseasConsulting() { // Removed { onBack } from props
  ...
  const countries = [
    { name: 'USA', universities: '4000+' },
    { name: 'UK', universities: '160+' },
    { name: 'Canada', universities: '100+' },
    { name: 'Australia', universities: '40+' },
    { name: 'Germany', universities: '380+' },
    { name: 'France', universities: '3500+' },
    { name: 'Singapore', universities: '30+' },
    { name: 'Ireland', universities: '20+' },
  ];
        ...
        <div className="grid md:grid-cols-4 gap-8">
          {steps.map((step, idx) => (
            <div key={idx} className="relative group">
              <div className="text-6xl font-black text-red-600 absolute -top-10 -left-2 group-hover:text-red-50 transition-colors">0{idx + 1}</div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {countries.map((country, index) => (
              <motion.div
                key={index}
                whileHover={{ y: -10, backgroundColor: "rgba(30, 41, 59, 1)" }}
                className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 transition-all cursor-pointer"
              >
                <div className="w-10 h-10 bg-red-700 rounded-lg flex items-center justify-center mb-4 shadow-lg shadow-red-900/20">
                  <MapPin size={20} className="text-white" />
                </div>
                <h3 className="text-lg font-bold mb-1">{country.name}</h3>
                <p className="text-slate-400 text-xs">{country.universities} Universities</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="bg-red-600 rounded-3xl p-12 text-white relative">
            ...
            <Button size="lg" className="bg-white text-red-600 hover:bg-red-50 w-full md:w-auto py-7 px-10 rounded-xl font-bold">
              Book Appointment
            </Button>
          </div>
        </div>
